...
function CodeEditorPaneInner({ pane }: CodeEditorPaneProps) {
  ...
  useEffect(() => {
    ...
    window.addEventListener('liveview-insert' as never, handler as never);
    return () => window.removeEventListener('liveview-insert' as never, handler as never);
  }, [pane]);
  ...
  useEffect(() => {
    ...
    window.addEventListener('liveview-format' as never, handler as never);
    return () => window.removeEventListener('liveview-format' as never, handler as never);
  }, [pane]);
  ...
  return (
    <div className="flex-1 overflow-hidden">
      <Editor
        ...
        onMount={handleMount}
        loading={
          <div className="flex items-center justify-center h-full ui-text-faint text-sm">
            Loading editor...
          </div>
        }
      />
